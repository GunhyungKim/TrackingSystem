...
// ==== Invoke Thermal ====
// peer chaincode invoke -o orderer0.example.com:7050 --tls --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C $CHANNEL_NAME -n thermal -c '{"Args":["initThermal","Name","PN","location","time","temperature"]}'
...
type thermal struct {
	...
	PN          string  `json:"phonenumber"`
	Location    string  `json:"location"`
	Time        string  `json:"time"`
	Temperature float64 `json:"temperature"`
	RawLC       string  `json:"rawlc"`
	RawT        string  `json:"rawt"`
	RawBT       string  `json:"rawbt"`
}
...
func (t *SimpleChaincode) initThermal(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	//   0        1        		 2       	 3				4
	// "Name", "PhoneNumber", "Location", "Time", "Body Temperature"
	if len(args) != 5 {
	...
	name := make([]byte, 14112, 14112)
	phonenumber := make([]byte, 12936, 12936)
	location := make([]byte, 3528, 3528)
	time := make([]byte, 11760, 11760)
	temperature := make([]byte, 4704, 4704)

	rawlc := args[1]
	rawt := args[2]
	rawbt, err := strconv.ParseFloat(args[3], 64)
	if err != nil {
	...
	objectType := "thermal"
	thermal := &thermal{objectType, name, phonenumber, location, time, temperature, rawlc, rawt, rawbt}
	...
	err = stub.PutState(rawlc+rawt, thermalJSONasBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (t *SimpleChaincode) queryThermalByLocation(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	queryString := fmt.Sprintf("{\"selector\":{\"rawlc\":\"%s\"}}", location)
...
func (t *SimpleChaincode) queryThermalByTemperature(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	queryString := fmt.Sprintf("{\"selector\":{\"rawbt\":{\"$gt\":%f}}}", temperature)
